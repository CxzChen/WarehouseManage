...
{
//    Countdown *cdReg,*cdForget,*cdOldCheck,*cdBindNew;
}
@end
...
- (BOOL)isNickname:(NSString*)text{
    if ([[text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] isEqualToString:@""] || [text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]].length < text.length) {
        return false;
    }
    
    NSString * regex = @"[a-zA-Z0-9\u4e00-\u9fa5\\s+]+";
...
//- (BOOL)isCodeByTel:(NSString*)text
//{
//    if ([[text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] isEqualToString:@""]) {
//        return false;
//    }
...
//            
//            /*
//            UIAlertController *alt = [UIAlertController alertControllerWithTitle:@"您的账号已在其他设备上登录，请重新登录" message:nil preferredStyle:UIAlertControllerStyleAlert];
...
//- (void)authInvalid{
//    if (_auth==nil) {
//        _auth=[AuthModel getModelFromDB];
//    }
//    if (_auth) {
//        _auth.username=nil;
//        _auth.vso_token=nil;
...
- (void)setCD:(BOOL)reset btn:(UIButton*)btn type:(TelCheckType)type {
    Countdown *cd;
    switch (type) {
        case TelCheckTypeRegister:
        {
            if (cdReg==nil) {
                cdReg=[Countdown aInstance];
            }
            cd=cdReg;
        }
            break;
        case TelCheckTypeOld:
        {
            if (cdOldCheck==nil) {
                cdOldCheck=[Countdown aInstance];
            }
            cd=cdOldCheck;
        }
            break;
        case TelCheckTypeNewBind:
        case TelCheckTypeChangeBind:
        {
            if (cdBindNew==nil) {
                cdBindNew=[Countdown aInstance];
            }
            cd=cdBindNew;
        }
            break;
        case TelCheckTypeForgetPwd:
        {
            if (cdForget==nil) {
                cdForget=[Countdown aInstance];
            }
            cd=cdForget;
        }
            break;
    ...
    if (reset){
        [cd setCD:kTelCDTime block:^(int numCD) {
            SyncBegin
            [btn setTitle:[NSString stringWithFormat:@"%lis",(long)numCD] forState:UIControlStateDisabled];
            if (numCD>0) {
                btn.enabled=NO;
            }
            else btn.enabled=YES;
            SyncEnd
        }];
    }
    else if(cd.curCD>0){
        cd.cdBlock = ^(int numCD) {
            SyncBegin
            [btn setTitle:[NSString stringWithFormat:@"%lis",(long)numCD] forState:UIControlStateDisabled];
            if (numCD>0) {
                btn.enabled=NO;

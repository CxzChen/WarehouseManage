...
@interface QYBaseTabBar()<UINavigationControllerDelegate>
{
...
- (UITabBarItem *)createTabBarItem:(NSString *)strTitle normalImage:(NSString *)strNormalImg selectedImage:(NSString *)strSelectedImg itemTag:(NSInteger)intTag
{
    ...
    [item setTitleTextAttributes:[NSDictionary dictionaryWithObjectsAndKeys:RGBHex(kColorGray204), NSForegroundColorAttributeName,fontSystem(kFontS22), NSFontAttributeName, nil] forState:UIControlStateNormal];
    [item setTitleTextAttributes:[NSDictionary dictionaryWithObjectsAndKeys:RGBHex(kColorGray204), NSForegroundColorAttributeName,fontSystem(kFontS22), NSFontAttributeName,  nil] forState:UIControlStateSelected];
...
- (void)addBadge:(NSArray*)tags{
    ...
    for (NSString *obj in tags) {
        frm.origin.x=ww*i-AutoValue(50.f);
//        DLog(@">>>addBadge rp:%@",NSStringFromCGRect(frm));
        UIImageView *imgIcon;
        imgIcon = [[UIImageView alloc] initWithFrame:frm];
        imgIcon.tag=obj.integerValue;
        imgIcon.layer.cornerRadius = imgIcon.height/2;
        imgIcon.layer.masksToBounds = YES;
        imgIcon.backgroundColor = RGBHex(kColorAuxiliary101);
        imgIcon.hidden = YES;

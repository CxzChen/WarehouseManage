...
- (NetError*)checkError:(id)error{
    NetError *err=nil;
    if ([error isKindOfClass:[NetError class]]) {
        return error;
    }
...
//                        DLog(@"JSON-%@: %@", [responseObject class],responseObject);
            ...
            [self checkResponse:responseObject success:success failure:failure];
            
        } failure:^(NSURLSessionTask *task, NSError *error) {
            [self checkResponse:error success:success failure:failure];
...
//            DLog(@"JSON: %@", responseObject);
        ...
        [_httpManager PUT:path parameters:params success:^(NSURLSessionDataTask * _Nonnull task, id  _Nonnull responseObject) {
            [self checkResponse:responseObject success:success failure:failure];
...
- (NSURLSessionDownloadTask *)downloadFile:(NSString*)fileUrl diskPath:(NSString*)diskPath reload:(BOOL)reload completion:(void (^)(NSURLResponse *response, NSString *filePath, NetError* err))completion writeDataBlock:(NCDownloadDataBlock)writeDataBlock{
    ...
    NSURLSessionDownloadTask *task = [http downloadTaskWithRequest:request progress:nil destination:^NSURL * _Nonnull(NSURL * _Nonnull targetPath, NSURLResponse * _Nonnull response){
        //        DLog(@"%@\n\n%@",targetPath,response    );
        //获取文件夹路径
        NSString *fn=[_filePath fileNameOfURL];
        NSString *dir=[_filePath substringWithRange:NSMakeRange(0, _filePath.length - fn.length)];
        
        //检查有没有这个文件夹,没有就创建
        if (![FileManager isExistDirectory:dir]) {
...
#pragma mark － upload 测试
- (NSURLSessionDataTask *)uploadWithPath:(NSString*)uploadPath file:(id)file name:(NSString*)name params:(NSDictionary *)params
               success:(void(^)(id responseObj))success failure:(void(^)(NetError *err))failure sendDataBlock:(NCSendDataBlock)sendDataBlock{
    NSString *mimeType = nil;
    NSData *data = nil;//
    if ([QGLOBAL object:file isClass:[NSData class]]) {
        data=file;
        mimeType=@"multipart/form-data";
    }
    else if ([QGLOBAL object:file isClass:[UIImage class]]) {
        data=UIImageJPEGRepresentation(file, .9);
        mimeType=@"image/jpeg";
        
//        data=UIImagePNGRepresentation(file);
//        mimeType=@"image/png";
    }
    else {
        if (failure) {
...
//   _httpManager.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"application/json", @"text/json", @"text/javascript", @"text/html",nil];//设置相应内容类型
    NSURLSessionDataTask * task= [_httpManager POST:uploadPath parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
//        [formData appendPartWithFormData:data name:@"attachment"];
//        [formData appendPartWithFileData:data name:@"attachment" fileName:@"attachment" mimeType:@"multipart/form-data"];
        [formData appendPartWithFileData:data name:@"attachment" fileName:fileName mimeType:mimeType];
        
        DLog(@"^^^^^^^^^^^ 上传文件大小: %li",[data length]);
    } success:^(NSURLSessionDataTask *task, id responseObject) {
        DLog(@">>>Success %@", responseObject);
        [self checkResponse:responseObject success:success failure:failure];

    } failure:^(NSURLSessionDataTask *task, NSError *error) {
        DLog(@">>>Upload Failure %@, %@", error, [task.response description]);
        if (failure) {
            failure([self checkError:error]);
        }
    }];
    ...
    NSMutableURLRequest *request = [_httpManager.requestSerializer multipartFormRequestWithMethod:@"POST" URLString:uploadPath parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        //注意这里的name和fileName 要和服务器匹配
        [formData appendPartWithFileData:data name:@"attachment" fileName:fileName mimeType:mimeType];
        
    } error:&serializationError];
    NSURLSessionUploadTask *task = [_httpManager uploadTaskWithStreamedRequest:request progress:nil completionHandler:^(NSURLResponse * __unused response, id responseObject, NSError *error) {
...
//        http=nil;
    ...
    */
    ...
    //跟踪load数据
    [_httpManager setTaskDidSendBodyDataBlock:^(NSURLSession * _Nonnull session, NSURLSessionTask * _Nonnull task, int64_t bytesSent, int64_t totalBytesSent, int64_t totalBytesExpectedToSend) {
        if (sendDataBlock) {
